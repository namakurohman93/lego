...
func loginToGameworld(c LobbyCookies, s, m, gw string) {
	gameworldId, err := getGameworldId(s, gw)
	if err != nil {
		log.Fatal(err)
	}
...
func getGameworldId(s, gw string) (r string, err error) {
	rc := request.NewRequestConfig()
